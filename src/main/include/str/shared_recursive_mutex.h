 ...
 */
// the template parameter is needed to be able to define multiple instances of
// the shared_recursive_mutex since the implementation relies upon thread local
// storage we need a unique type per lock that is needed is doesn't matter what
// the input type is, along as it's unique (that's why it's called PhantomType)
template <typename PhantomType> class shared_recursive_mutex_t {
public:
  /**
  ...
  /**
   * @brief The shared_recursive_mutex_t is relying on thread local storage, so
   * there can only be 1 valid instance of it
   */
   ...
   *              Ownership will only be released after the thread makes a
   ...
   *        * other threads try to get write access
   *
   *        A thread may call lock repeatedly. If the thread already has write
   * access the level of write access will be increased. Ownership will only be
   * released after the thread makes a matching number of calls to
   * unlock_shared.
   */
  void lock_shared();

  /**
   * @brief Unlocks the mutex for this thread if its level of write ownership is
   * 1 and has no read ownership. If the thread has write ownership of 1 and
   * read ownership, the mutex will change from write to read access. Otherwise
   * reduces the level of ownership by 1.
   */
  void unlock();

  /**
   * @brief Unlocks the mutex for this thread if its level of ownership is 1.
   * Otherwise reduces the level of ownership by 1.
   */
  void unlock_shared();
  /**
   * @brief Tries to get write ownership if possible. If the thread has read
   * (but no write) ownership this function returns false, because to upgrade a
   * read lock to a write lock we have to give up read ownership, so if we can't
   * aquire write ownership we have to reaquire the read ownership again, which
   * might be a blocking operation. Use try_lock_upgrade is this is the wanted
   * behavior.
   */
...
{
  // if the g_writers are > 0 it means that when we got the read lock, this
  // thread already has the write lock
  if (g_writers > 0) {
    unlock();
    return;
  }
...
bool shared_recursive_mutex_t<PhantomType>::try_lock()
{
  ...
  // we already have a read lock, but we can't aquire the write lock without
  // giving up the read lock so we have to return false here
  if (g_readers > 0) {
    return false;
  }
...
using shared_recursive_global_mutex
  = shared_recursive_mutex_t<struct AnonymousType>;
