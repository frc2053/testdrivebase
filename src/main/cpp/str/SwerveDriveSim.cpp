...
SwerveDriveSim::SwerveDriveSim(const frc::LinearSystem<2, 1, 2>& drivePlant,
  units::volt_t driveKs, const frc::DCMotor& driveMotor, double driveGearing,
  units::meter_t driveWheelRadius, const frc::LinearSystem<2, 1, 2>& steerPlant,
  units::volt_t steerKs, const frc::DCMotor& steerMotor, double steerGearing,
  const frc::SwerveDriveKinematics<numModules>& kinematics)
...
void SwerveDriveSim::Update(units::second_t dt)
{
  ...
  frc::DiscretizeAB<2, 1>(
    drivePlant.A(), drivePlant.B(), dt, &driveDiscA, &driveDiscB);

  Eigen::Matrix<double, 2, 2> steerDiscA;
  Eigen::Matrix<double, 2, 1> steerDiscB;
  frc::DiscretizeAB<2, 1>(
    steerPlant.A(), steerPlant.B(), dt, &steerDiscA, &steerDiscB);

  std::array<frc::SwerveModulePosition, 4> moduleDeltas;

  for (int i = 0; i < numModules; i++) {
    double prevDriveStatePos = driveStates[i](0, 0);
    driveStates[i] = CalculateX(
      driveDiscA, driveDiscB, driveStates[i], driveInputs[i], driveKs);
    double currentDriveStatePos = driveStates[i](0, 0);
    steerStates[i] = CalculateX(
      steerDiscA, steerDiscB, steerStates[i], steerInputs[i], steerKs);
...
void SwerveDriveSim::Reset(const frc::Pose2d& pose,
  const std::array<Eigen::Matrix<double, 2, 1>, numModules>& moduleDriveStates,
  const std::array<Eigen::Matrix<double, 2, 1>, numModules>& moduleSteerStates)
{
  this->pose = pose;
...
std::array<frc::SwerveModulePosition, numModules>
SwerveDriveSim::GetNoisyModulePositions(
...
std::array<frc::SwerveModuleState, numModules> SwerveDriveSim::GetModuleStates()
{
...
units::ampere_t SwerveDriveSim::GetCurrentDraw(const frc::DCMotor& motor,
...
std::array<units::ampere_t, numModules>
