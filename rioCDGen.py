...
argParser.add_argument(
    ...
    default="Debug",
    help="The type of build. Can be Debug or Release. Defaults to Debug",
)
argParser.add_argument(
    "--year",
    "-y",
    type=int,
    help="The year of the toolchain/project. Defaults to the current year",
...
argParser.add_argument(
    ...
    "-s",
    type=str,
    help="The root directory of the project(should be the directory with your build.gradle file. Defaults to cwd",
    default=os.getcwd(),
)
...
if not os.path.exists(args.source):
    ...
    print(
        "project directory does not contain a build.gradle! Is this a project directory?"
    )
    exit(1)
else:
...
def findCompiler() -> str:
    ...
    name = os.path.sep + os.path.join(
        f"{year}", "roborio", "bin", f"arm-frc{year}-linux-gnueabi-g++"
    )
...
def findOptionsTXT() -> str():
    name = os.path.join(
        f"compileFrcUserProgramLinuxathena{buildType}ExecutableFrcUserProgramCpp",
        "options.txt",
    )
